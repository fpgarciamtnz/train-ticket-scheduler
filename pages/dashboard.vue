...
async function saveProfile() {
  ...
  if (profilePin.value && profilePin.value !== profileConfirmPin.value) {
    profileError.value = 'PINs do not match'
    return
  }

  if (profilePin.value && !/^\d{4}$/.test(profilePin.value)) {
  ...
  try {
    // For now, profile editing would require a dedicated endpoint
    // We'll just show the info
    ...
    <UTabs :items="tabItems">
      ...
      <template #profile>
        <div class="pt-4 max-w-md space-y-6">
          <div>
            <h3 class="font-medium text-gray-900 dark:text-gray-100 mb-2">Your Profile</h3>
            <div class="space-y-3">
              <div>
                <label class="block text-sm font-medium mb-1 text-gray-900 dark:text-gray-100">Name</label>
                <UInput :model-value="auth.user?.name" disabled />
              </div>
              <div>
                <label class="block text-sm font-medium mb-1 text-gray-900 dark:text-gray-100">Email</label>
                <UInput :model-value="auth.user?.email" disabled />
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Used for login and receiving notifications about new requests.</p>
              </div>
            </div>
          </div>

          <div>
            <h3 class="font-medium text-gray-900 dark:text-gray-100 mb-2">Your Calendar URL</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">Share this link so others can view your calendar and submit requests:</p>
            <div class="flex gap-2 items-center">
              <UInput :model-value="`${useRequestURL().origin}/u/${auth.user?.slug}`" readonly class="flex-1" />
            </div>
          </div>
        </div>
      </template>
